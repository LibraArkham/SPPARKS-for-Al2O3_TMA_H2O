...
def run(filename:str,outfile:str,strict_distance:float, stric_z:float, facet:tuple, layers:int, supercell:tuple, element_dict=None):
    ...
    cif2pos(filename,'slab.vasp',facet,layers,supercell)
    data_dic = read('slab.vasp')
...
def cif2pos(cif_file, pos_file, facet, layers, supercell):
    ...
    # 4. 输出切割后的结构
    io.write('slab.xyz', slab)  # 将切好的晶面保存为新的xyz文件
    io.write('slab.cif', slab)  # 将切好的晶面保存为新的cif文件
    io.write('slab.vasp',slab_sort,direct=True)
    # element_dict = {'Al':2,'O':1} # 元素对应的数字表示

if __name__ == '__main__':
    outfile = 'data.ald' # 输出文件
    cif_file =  'Al2O3.cif' # 输入文件
    facet = (0,0,1)# 晶面
    layers =  5 # 层数
    supercell = (5,5,1)# 扩胞
    sub_layers =  1 # subatrate
